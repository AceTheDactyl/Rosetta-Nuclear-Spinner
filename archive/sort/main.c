...
static void System_SendTelemetry(void);
...
static void System_MainLoop(void) {
    ...
    // Send telemetry (100 Hz)
    if (now - g_timing.last_telemetry >= (1000 / TELEMETRY_RATE_HZ)) {
        g_timing.last_telemetry = now;
        System_SendTelemetry();
    }
}
...
static void System_ProcessCommands(void) {
    /**
     * Command protocol (simplified for firmware demonstration):
     * 
     * Header byte + data
     * 'Z' + float: Set z target
     * 'P' + float + float + uint32: Send pulse (amp, phase, duration)
     * 'E' + uint8: Enable (1) / Disable (0) motor
     * 'C' + uint8: Start calibration
     * 'X' + uint8: Start experiment (type)
     * 'S': Stop current operation
     * 'Q': Query status
     */
    
    // This would read from UART/USB
    // Placeholder for actual serial implementation
    
    // Example: Check if any data available (HAL would provide this)
    // uint8_t cmd = HAL_Serial_Read();
    // switch (cmd) { ... }
}
...
static void System_SendTelemetry(void) {
     ...
     *   - float: kappa
     *   - float: eta
     *   - uint8: available_operators
     *   - bool: k_formation_active
     *   - float: temperature
     * Checksum: uint16
     */
    
    // This would serialize and send via UART/USB
    // Placeholder for actual telemetry implementation
    
    // The host software (Python API) would parse this
}
